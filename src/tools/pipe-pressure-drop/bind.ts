...
export function bindPipePressure(runtime: ToolRuntime): void {
  ...
  const syncFlowUnit = (): void => { const basis = stringValue("pipe-flow-basis"); const unit = basis === "mass" ? "kg/h" : "m3/h"; const select = form.querySelector<HTMLSelectElement>("#pipe-flow-unit"); if (select) { select.innerHTML = `<option value="${unit}">${unit === "m3/h" ? "m³/h" : "kg/h"}</option>`; select.value = unit; } };
  ...
  form.querySelector("#pipe-flow-basis")?.addEventListener("change", syncFlowUnit);
